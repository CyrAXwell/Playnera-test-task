...
public class EyeShadowState : IHandStrategy
{
    ...
    public EyeShadowState(EyeShadowBrush eyeShadowBrush, Hand hand, Canvas canvas, RectTransform handStartPosition)
    {
    ...
    public void PickUpTargetItem(MakeupItemView itemView)
    {
        ...
        if (!_isHoldItem)
        {
            targetPosition = _brush.transform.position - (_hand.EyeShadowkWayPoints.PickUpPoint - _hand.transform.position);
        ...
        duration = 0.4f;
        Vector3 ItemViewPickUpPoint = _itemView.transform.position - (_hand.EyeShadowkWayPoints.ItemViewPickUpPoint - _hand.transform.position);
        ...
        duration = 0.5f;
        targetPosition = _hand.EyeShadowkWayPoints.HoldPoint / _canvas.scaleFactor;
    ...
    {
        EyeShadowItemSO item = (EyeShadowItemSO)_itemView.MakeupItem;
        _brush.ChangeColor(item.TipColor);
    }

    private void PickUp()
    {
        _brush.transform.SetParent(_hand.EyeShadowkWayPoints.transform);
    }
    ...
    public void UsePickUpItem()
    {
        ...
        for (int i = 0; i < 2; i++)
        {
            foreach (var point in _hand.EyeShadowkWayPoints.MakeupPoints)
            {
